...
# Summary of code used in WastewaterRe Paper by Huisman et. al (file wastewater_Zurich.R)
...
# Deconvolution of Wastewater Data

## - Normalisation of Wastewater Data
- Note that this follows after the initial data manipulations presented above.
```{r}
...
- The outer loop goes over the two different types of delay distributions. Essentially, they scan over the 600 different SLD paramter combinations twice. 
- We could adapt this loop for our purposes in order to loop over variants and cantons.
```{r eval = FALSE}
 ...
 for (incubationParam in c('zero', 'incubation')){
   for (incidence_var in c('norm_n1')){
     for (canton in c('ZH')){
       ...
       # plotData is modified version of Re_cases
...
row_id <- 1
incubationParam <-'zero'
incidence_var <- 'norm_n1'
canton <- 'ZH'
...
deconv_results = cbind(expand_grid(meanOpts, sdOpts),
                              'rmse_cc' = NA, 'coverage_cc' = NA, 'mape_cc' = NA,
                              'rmse_h' = NA, 'coverage_h' = NA, 'mape_h' = NA)
# Check how deconv_results looks
deconv_results
      

# Input parameters delay distribution
getCountParams(incubationParam)
                  
# Input parameters  SLD
getGammaParams(deconv_results[row_id, 'meanOpts'], deconv_results[row_id, 'sdOpts'])
...
# go to next iteration in case there is some error message
if('try-error' %in% class(deconv_config)){
           deconv_results[row_id, c('rmse_cc', 'coverage_cc', 'mape_cc')] = c(Inf, 0, Inf)
           deconv_results[row_id, c('rmse_h', 'coverage_h', 'mape_h')] = c(Inf, 0, Inf)
           next
...
# Obtain R value from deconvolved series
Re_config = getReBootstrap(deconv_config)
...
deconv_results 
#write_csv(deconv_results, paste0('../scan/deconv_', incubationParam, '_',canton, '_', incidence_var, '.csv'))
...
compareTraces(Re_config, plotData_subset %>%
                                                                                          filter(data_type == 'Confirmed cases'))
```
